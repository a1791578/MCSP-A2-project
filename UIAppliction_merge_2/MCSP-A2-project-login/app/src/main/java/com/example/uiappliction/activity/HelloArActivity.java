...
import com.example.uiappliction.Utils.helpers.CameraPermissionHelper;
import com.example.uiappliction.Utils.helpers.DepthSettings;
import com.example.uiappliction.Utils.helpers.DisplayRotationHelper;
import com.example.uiappliction.Utils.helpers.FullScreenHelper;
import com.example.uiappliction.Utils.helpers.InstantPlacementSettings;
import com.example.uiappliction.Utils.helpers.SnackbarHelper;
import com.example.uiappliction.Utils.helpers.TapHelper;
import com.example.uiappliction.Utils.helpers.TrackingStateHelper;
import com.example.uiappliction.Utils.samplerender.Framebuffer;
import com.example.uiappliction.Utils.samplerender.Mesh;
import com.example.uiappliction.Utils.samplerender.SampleRender;
import com.example.uiappliction.Utils.samplerender.Shader;
import com.example.uiappliction.Utils.samplerender.Texture;
import com.example.uiappliction.Utils.samplerender.VertexBuffer;
import com.example.uiappliction.Utils.samplerender.arcore.BackgroundRenderer;
import com.example.uiappliction.Utils.samplerender.arcore.PlaneRenderer;
import com.example.uiappliction.Utils.samplerender.arcore.SpecularCubemapFilter;
...
/*
 * Portions of this file include code from Google's ARCore open-source project.
 * Source repository: https://github.com/google-ar/arcore-android-sdk
 * Copyright belongs to Google, under the Apache 2.0 license.
 * For more information, please refer to the LICENSE file in the project.
 */
public class HelloArActivity extends AppCompatActivity implements SampleRender.Renderer {
  ...
  public void onSurfaceCreated(SampleRender render) {
    Log.d(TAG, "onSurfaceCreated: Start loading 3D models");
    // 准备渲染对象。这涉及读取着色器和3D模型文件，因此可能会抛出 IOException。
    try {
      ...
      Log.e(TAG, "读取必要资源文件失败", e);
      messageSnackbarHelper.showError(this, "读取必要资源文件失败: " + e);
    }
  }

  private void loadDfgTexture(SampleRender render) throws IOException {
  ...
  //每一帧调用update，用session update来获取当前的frame
  @Override
  public void onDrawFrame(SampleRender render) {
    ...
    if (hasPlacedObject && virtualObjectMesh != null) {
      ...
      if (dimensions != null) {
        String dimensionsText = String.format("length: %.2fm, width: %.2fm, hight: %.2fm",
                dimensions[0], dimensions[1], dimensions[2]);

        runOnUiThread(() -> {
          TextView dimensionTextView = findViewById(R.id.dimensionTextView);
          dimensionTextView.setText(dimensionsText);
        });
      }
    } else {
      runOnUiThread(() -> {
        TextView dimensionTextView = findViewById(R.id.dimensionTextView);
        dimensionTextView.setText("");
      });
    }
  ...
  private void deletePlacedModel() {
    for (WrappedAnchor wrappedAnchor : wrappedAnchors) {
      Anchor anchor = wrappedAnchor.getAnchor();
      if (anchor != null) {
        anchor.detach();
